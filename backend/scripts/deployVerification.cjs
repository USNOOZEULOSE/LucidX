...
async function main() {
  ...
  const verificationFactory = await hre.ethers.getContractFactory("Verification");
  const verificationInstance = await verificationFactory.deploy({ gasLimit: 100000000 });
